angular.module("series").service("ServiceRest" , function($http,$state){
	...
	var _getUser = function(usera){
		console.log(usera);
		user = {};
		var promise = $http.post("/api/busca", usera);
		promise.then(function(as){
			this.user = as.data;
			$state.transitionTo("series");
	...
	var _adicionarMinhasSeries = function(serie, id){
		var promise = $http.post("/api/"+id+"/watched",serie);
		promise.then(function(as){
	...
	var _adicionarWatchList = function(serie, id){
		var promise = $http.post("/api/"+id+"/watch",serie);
		promise.then(function(as){
	...
	var _removeWatched = function(id, i){
		var promise = $http.delete("/api/"+id+"/minhasseries/"+i);
		promise.then(function(as){
			return as.data;
		});
	}
	...
	var _getSeries = function(){
			var promise = $http.get("/api/"+usuario.data.id+"/minhasseries");
			return promise;
	}
	
	var _addNota = function(id, i, nota){
		var promise = $http.put("/api/"+id+"/minhasseries/"+i, nota);
		return promise;
	}
